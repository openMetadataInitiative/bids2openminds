...
def test_example_datasets_click():
    ...
    runner = CliRunner()
    result = runner.invoke(convert_click, [test_dir, "--short-name", "False"])
...
def test_example_datasets_click_seperate_files():
    ...
    result = runner.invoke(
        convert_click, ["--multiple-files", test_dir, "--short-name", "False"])
    assert result.exit_code == 0
...
def test_example_datasets_click_output_location():
    ...
    runner = CliRunner()
    result = runner.invoke(
        convert_click, ["-o", openminds_file, test_dir, "--short-name", "False"])
    assert result.exit_code == 0
    c = Collection()
